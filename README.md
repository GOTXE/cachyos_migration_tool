...
| **Restic backup** | Prepara y ejecuta backup continuo por SFTP/SSH con fallback LAN/remoto y timer de usuario |
...
El timer de `restic-backup` sigue comprobando cada 30 minutos, pero por defecto solo ejecuta copias entre `08:00` y `22:00`. Ese horario se ajusta en `~/.config/cachyos-migration-tool/backup.env` con `BACKUP_ALLOWED_START_HOUR`, `BACKUP_ALLOWED_END_HOUR` y `BACKUP_ENFORCE_SCHEDULE`.

El restore normaliza permisos por defecto: ficheros nuevos quedan en `644`, directorios en `755` y los repositorios Git recuperan el bit ejecutable sólo para archivos marcados como `100755` en su índice. Usa `--preserve-permissions` sólo si necesitas restaurar literalmente los modos del backup.
...
`post-restore-fixups` sirve para normalizar un `$HOME` restaurado en otra máquina: recompone CLIs npm/NVM (`codex`, `gemini`, `pnpm`), instala dependencias locales de `talk2ai`, elimina autostarts heredados de `codexBar Tray`, limpia locks de Brave y repara la dependencia `google-auth-library` de Antigravity IDE si falta.
...
| `androidstudio` | Instala Android Studio + JDK 21 |
...
- `./migration.sh restic-backup ...` gestiona el repositorio Restic y el timer de backup de usuario, con ventana horaria configurable
...
- [Backup Restic por SFTP/SSH](docs/backup-sftp-restic.md)
- [Monitor de backup para plasmoid MBP Watch](docs/mbp-watch/backup-monitor.md)
